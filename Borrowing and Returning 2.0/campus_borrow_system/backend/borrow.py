...
def create_borrow_request(user_id, equipment_id, borrow_date=None, quantity_requested=1, expected_return_date=None):
    """
...
def list_current_borrows():
    ...
    try:
        ...
        try:
            cursor.execute("""
                SELECT br.request_id, br.user_id, u.username, br.equipment_id, ei.equipment_name, br.borrow_date, br.expected_return_date
                FROM borrow_request br
                JOIN user_login u ON br.user_id = u.user_id
                JOIN equipment_inventory ei ON br.equipment_id = ei.equipment_id
