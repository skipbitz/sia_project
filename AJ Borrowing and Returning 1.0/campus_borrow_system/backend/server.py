    ...
    def handle_api_post(self, path):
        ...
        if path == '/api/borrow':
            ...
            try:
                ...
                equip = None
            # capture equipment_name early so we can include it in the response
            equip_name = equip.get('equipment_name') if equip else None
            if not equip:
            ...
            try:
                ...
                bid = borrow_mod.create_borrow_request(sess['user_id'], eid, borrow_date=borrow_date, quantity_requested=quantity, expected_return_date=return_date, reason=reason)
                ...
                try:
                    ...
                    pass
                return json_response(self, {'error': 'failed to create borrow request', 'details': str(e)}, status=500)
            return json_response(self, {'request_id': bid, 'equipment_name': equip_name})
        ...
        if path == '/api/return':
            ...
            # use the conditional-aware create function if available
            try:
                rid = return_mod.create_return_request_with_condition(borrow_id, condition=condition)
            except Exception:
...
def run(host='127.0.0.1', port=8000):
    ...
    try:
        ...
        pass
    try:
        # ensure return_request has optional columns (like 'condition')
        import backend.return_mod as _rm
        try:
            _rm.ensure_return_request_columns()
        except Exception:
            # if function missing, ignore
            pass
    except Exception:
        pass
