...
async function loadMyRequests(){
  ...
  d.innerHTML = `<div><strong>Request #${r.request_id}</strong> — ${equipName} — <em>${r.status}</em></div>${reason?`<div class="small">Reason: ${reason}</div>`:''}`
    // if approved, allow user to create a return request
    if(r.status === 'approved'){
      const ret = el('button','btn small','Return')
      ret.style.marginLeft='12px'
      // open return modal and populate borrow id
      ret.onclick = ()=>{
        const modal = document.getElementById('returnModal')
        const input = document.getElementById('return_borrow_id')
        if(input) input.value = r.request_id
...
async function loadRecentBorrowsUser(){
  try{
    ...
    rows.forEach(r=>{
      ...
      const reason = r.reason || ''
      tr.innerHTML = `<td>${r.request_id}</td><td>${r.equipment_name||r.equipment_id}</td><td>${reason}</td><td>${r.borrow_date||''}</td><td>${r.status||''}</td>`
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(borrowModal){
    ...
    borrowForm?.addEventListener('submit', async (ev)=>{
      ...
      try{
        const reason = (document.getElementById('borrow_reason')?.value || '').toString().trim()
        const resp = await fetchJSON('/api/borrow', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({equipment_id, quantity, borrow_date: borrow_datetime, return_date: return_datetime, reason})})
        if(resp && resp.error){ alert(resp.error); return }
        // If backend returns equipment_name, show it in confirmation
        if(resp && resp.equipment_name){
          alert(`Borrow request submitted for: ${resp.equipment_name} (Request #${resp.request_id})`)
        }else{
  ...
  if(returnModal){
    ...
    returnForm?.addEventListener('submit', async (ev)=>{
      ev.preventDefault()
      const borrow_id = parseInt(document.getElementById('return_borrow_id').value)
      // read selected condition
      const condition = document.getElementById('return_condition')?.value || 'good'
      const notes = (document.getElementById('return_notes')?.value || '').toString().trim()
      try{
        const resp = await fetchJSON('/api/return', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({borrow_id, condition, notes})})
        if(resp && resp.error){ alert(resp.error); return }
        ...
        loadEquipment()
        loadRecentBorrowsUser()
      }catch(err){ console.error(err); alert('Failed to submit return request') }
    })
  }
})
