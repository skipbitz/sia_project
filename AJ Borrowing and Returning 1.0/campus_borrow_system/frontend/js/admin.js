...
async function loadBorrowRequests(){
  ...
  if(tableBody){
    ...
    rows.forEach(r=>{
      ...
      tr.innerHTML = `<td>${r.request_id}</td><td>${username}</td><td>${equipment}</td><td>${r.borrow_date||''}</td><td>${returnDate}</td><td>${reason}</td><td>${r.status}</td><td>${actionsHtml}</td>`
...
async function loadReturnRequests(){
  ...
  if(tableBody){
    ...
    rows.forEach(r=>{
      ...
      const notes = r.notes || ''
      // show condition and optional notes under it
      const condHtml = escapeHtml(condition) + (notes ? `<div class="small">${escapeHtml(notes)}</div>` : '')
      tr.innerHTML = `<td>${r.return_id}</td><td>${r.borrow_id}</td><td>${escapeHtml(username)}</td><td>${escapeHtml(equipment)}</td><td>${condHtml}</td><td>${r.return_date||''}</td><td>${escapeHtml(r.status)}</td><td><button class='btn approveReturn' data-id='${r.return_id}'>Approve</button></td>`
      tableBody.appendChild(tr)
    })
    ...
    rows.forEach(r=>{
      ...
      const condition = r.condition || ''
      const notes = r.notes || ''
      d.innerHTML = `#${r.return_id} borrow ${r.borrow_id} - ${r.status} <div class="small">Condition: ${escapeHtml(condition)}${notes?('<br/>' + escapeHtml(notes)):''}</div>`
...
async function loadRecentBorrows(){
  try{
    ...
    rows.forEach(r=>{
      const tr = document.createElement('tr')
      const reason = r.reason || ''
      tr.innerHTML = `<td>${r.request_id}</td><td>${r.username||r.user_id}</td><td>${r.equipment_name||r.equipment_id}</td><td>${reason}</td><td>${r.borrow_date||''}</td><td>${r.status||''}</td>`
...
async function loadCurrentBorrows(){
  try{
    ...
    rows.forEach(r=>{
      ...
      const returnDate = r.expected_return_date || r.return_date || ''
      const reason = r.reason || ''
      tr.innerHTML = `<td>${r.request_id}</td><td>${r.username||r.user_id}</td><td>${r.equipment_name||r.equipment_id}</td><td>${reason}</td><td>${r.borrow_date||''}</td><td>${returnDate}</td>`
...
async function loadBorrowHistory(){
  try{
    ...
    rows.forEach(r=>{
      const tr = document.createElement('tr')
      const reason = r.reason || ''
      tr.innerHTML = `<td>${r.history_id}</td><td>${r.request_id||''}</td><td>${r.username||r.user_id||''}</td><td>${r.equipment_name||r.equipment_id||''}</td><td>${reason}</td><td>${r.borrow_date||''}</td><td>${r.return_date||''}</td>`
      tbody.appendChild(tr)
    })
  }catch(err){
