...
async function loadEquipment(){
  ...
  rows.forEach(r=>{
    ...
    const btn = el('button','', 'Request Borrow')
    btn.onclick = async ()=>{
      await fetchJSON('/api/borrow',{method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({equipment_id:r.equipment_id})})
